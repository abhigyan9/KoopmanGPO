...
def generate_initial_conditions(system, num_trajectories):
    if system == "Unforced Duffing":
        x0 = torch.tensor(np.random.uniform(1.5, 2.5, size=(1, num_trajectories)), dtype=torch.float64)
        x1 = torch.tensor(np.random.uniform(-1.5, 1.5, size=(1, num_trajectories)), dtype=torch.float64)
        ...
        x0 = torch.tensor(np.random.uniform(-1., 1., size=(1, num_trajectories)), dtype=torch.float64)
        x1 = torch.tensor(np.random.uniform(-1., 1., size=(1, num_trajectories)), dtype=torch.float64)
        return torch.vstack([x0, x1])

    elif system == "Simple Pendulum":
        x0 = torch.tensor(np.random.uniform(-2., 2., size=(1, num_trajectories)), dtype=torch.float64)
        x1 = torch.tensor(np.random.uniform(-3., 3., size=(1, num_trajectories)), dtype=torch.float64)
        return torch.vstack([x0, x1])
        ...
        x0 = torch.tensor(np.random.uniform(0., 2., size=(1, num_trajectories)), dtype=torch.float64)
        x1 = torch.tensor(np.random.uniform(0., 1., size=(1, num_trajectories)), dtype=torch.float64)
        return torch.vstack([x0, x1])
    ...
    # --- New systems below ---
        ...
        x0 = torch.tensor(np.random.uniform(-1.5, 1.5, size=(1, num_trajectories)), dtype=torch.float64)
        x1 = torch.tensor(np.random.uniform(-1.5, 1.5, size=(1, num_trajectories)), dtype=torch.float64)
        return torch.vstack([x0, x1])

    elif system == "Inhibited Predator-Prey":
        # Prey/predator strictly positive
        prey0 = torch.tensor(np.random.uniform(0.1, 4.0, size=(1, num_trajectories)), dtype=torch.float64)
        pred0 = torch.tensor(np.random.uniform(0.1, 3.0, size=(1, num_trajectories)), dtype=torch.float64)
...
def generate_and_save_data():
    ...
    systems = {
        ...
        # "Lorenz":               (gpk.f_Lorenz,         3, 0.01, torch.tensor([10., 8./3., 28.], dtype=torch.float64), 3000, 40, False),
    ...
    for system_name, (fx, state_dim, ts, params, num_steps, num_trajectories, is_discrete) in systems.items():
        ...
        for j in range(num_trajectories):
            x0 = x0_mat[:, j]
            if is_discrete:
                states = gpk.sim_discrete(fx, x0, ts, num_steps + 1, params=params)
            else:
        ...
        data = {
            "trajectories": torch.stack(trajectories),         # shape: [num_trajectories, state_dim, num_steps+1]
            "initial_conditions": torch.stack(initial_conditions),  # shape: [num_trajectories, state_dim]
            "sample_time": ts,
            "num_steps": num_steps,
