...
def result_filename(args):
    return f"seed_z-{safe_token(args.seed_z)}_seed_hp-{safe_token(args.seed_hp)}.json"


def default_outdir(system_name, datestamp):
    return Path("Figures") / "SeedSensitivity" / f"{system_name}_{datestamp}"
...
def build_dataset(args):
    ...
    if NORMALIZE_DATA:
        ...
        sim_data_clean = sim_data_raw.to(dtype=torch.float32)
    
    sim_data_clean = torch.flip(sim_data_clean, dims=[0])
    ...
    metadata = {
        ...
        "test_frac": float(1.0 - args.train_frac),
...
def run_single_seed_pair(args, out_path):
    ...
    print("--------------------------------------------")
    print(
        f"Seed sensitivity run | z_seed={args.seed_z}, hp_seed={args.seed_hp}, "
        f"traj_batch_size={b_label}"
    )
    ...
    cost_history = results["history"].get("cost", [])
    pre_mle_cost = results["history"].get("best_full_cost", None)
    if pre_mle_cost is None:
        pre_mle_cost = cost_history[-1]
    ...
    row = {
        ...
        **metadata,
        ...
        "z_seed": int(args.seed_z),
        "hp_seed": int(args.seed_hp),
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run one iGPK seed-sensitivity seed pair."
    )
    parser.add_argument("--seed-z", type=int, required=True)
    parser.add_argument("--seed-hp", type=int, required=True)
    ...
    parser.add_argument("--stop-tol", type=float, default=STOP_TOL)
    parser.add_argument("--routine", default=ROUTINE)
    parser.add_argument("--traj-batch-size", type=parse_traj_batch_size, default=TRAJ_BATCH_SIZE)
...
def main():
    ...
    try:
        payload = run_single_seed_pair(args, out_path)
